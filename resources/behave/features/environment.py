...
def before_all(context):
    LogManager.init_global_logger(context)
    StatsManager.init_stats(context)
    logging.info("Inicio de pruebas")

def before_feature(context, feature):
    context.generate_evidence = os.getenv("GENERATE_EVIDENCE", "false").lower() == "true"
    LogManager.setup_feature_logger(context, feature)
...
def before_scenario(context, scenario):
    ...
    context.driver = DriverManager.create_driver(headless_mode)
    ...
    DatasetManager.load_dataset(context)
    EvidenceManager.setup_evidence(context, scenario)
    LogManager.setup_scenario_logger(context, scenario)
...
def after_step(context, step):
    LogManager.log_step_diagnostics(context, step)
...
def after_scenario(context, scenario):
    ...
    StatsManager.update_stats(context, scenario)
    ...
    if context.generate_evidence:
        ...
        if hasattr(context, 'txt_filename') and hasattr(context, 'start_time'):
            ...
            ReportManager.generate_scenario_report(context, scenario, end_time, screenshots)

def after_feature(context, feature):
    LogManager.consolidate_feature_logs(context, feature)

def after_all(context):
    ReportManager.generate_consolidated_report(context)
