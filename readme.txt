...
● **JavaScript:** `javascript-obfuscator` vía `npx` (p. ej. salida `recorder.obfuscated.js` o `recorder.test.js`). En runtime se busca, en orden: variable `BEE_RECORDER_JS`, `recorder.obfuscated.js`, `recorder.test.js`, otros `recorder.*.js`, y `recorder.js` plano. **PyInstaller** debe incluir al menos uno de esos archivos en `core/` (`BEE.spec` empaqueta todos los `core/recorder*.js`).
