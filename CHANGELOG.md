...
- Importación avanzada de datos para pruebas masivas (CSV y Excel): Se rediseñó por completo el flujo de gestión de datos dentro del panel de suites y carga, habilitando una zona interactiva para arrastrar o buscar archivos en formatos .csv y .xlsx. Las hojas de cálculo de Excel se convierten de manera automática a partir de su primera pestaña y se almacenan de forma segura en el almacenamiento local del proyecto. Esta función incorpora una completa vista previa de filas y columnas, alertas de confirmación al sobrescribir archivos existentes, un acceso rápido para abrir la carpeta de datos en tu equipo y una pestaña secundaria integrada para editar los datos de forma manual directamente desde la interfaz.
- Asistente de migración de pruebas de rendimiento (JMeter): Nueva herramienta interactiva dentro del panel de Suites y carga que permite arrastrar planes de prueba tradicionales de rendimiento para transformarlos automáticamente en un reporte de estructura compatible (peticiones HTTP, encabezados, variables de entorno, archivos de datos CSV vinculados y extractores de expresiones regulares), sugiriendo de inmediato el perfil de carga óptimo. El sistema identifica elementos no soportados nativamente y los lista de forma clara como advertencias de diseño, abstrayendo toda la configuración para crear una colección completamente editable dentro del Cliente API.
- Conversión avanzada y traducción asistida de flujos de carga: Tras la importación del plan de rendimiento externo, el sistema genera automáticamente una secuencia encadenada de flujos lógicos y almacena metadatos de simulación, habilitando el botón «Aplicar sugerencia de rendimiento». Incluye una opción avanzada para forzar la inclusión de controladores deshabilitados en el archivo original, traducción asistida de scripts lógicos simples a formatos compatibles con el Cliente API, y el aislamiento preventivo en comentarios de bloques de código complejos. Además, cuenta con un sistema de limpieza automatizado que remueve de forma transparente los metadatos y flujos asociados en el momento en que decides eliminar la colección o los escenarios importados de la interfaz.
- Pantalla de bienvenida interactiva y arranque optimizado: Introducción de una pantalla de carga de marca con una narrativa visual estructurada en 4 fases dinámicas (inicialización del núcleo, validación de accesos, sincronización de módulos y estado listo). El panel muestra barras de progreso segmentadas, indicadores circulares animados y pilares de estado que se encienden progresivamente, desapareciendo mediante una sutil transición de desvanecimiento (180 ms) en el instante exacto en que la aplicación responde, garantizando un inicio ágil sin tiempos de espera mínimos forzados. El componente respeta estrictamente las preferencias del sistema operativo respecto a la reducción de movimiento. Además, se configuró un nuevo tema visual oscuro profundo y elegante como predeterminado para las nuevas instalaciones, asegurando la óptima distribución y empaquetado de los recursos gráficos integrados.
- Identidad visual expandida y Modo HUD opcional: Rediseño estético de la interfaz de carga que integra elementos visuales de última generación, trazas de circuitos impresos, logotipos compactos y pilares dedicados a los ejes de la plataforma (Seguridad, Inteligencia, Automatización y Evolución). Se incorporó un nuevo modo de visualización de alta tecnología (HUD) activable de forma opcional desde el menú de Configuración → General, así como estados lumínicos acotados y estilizados para identificar visualmente en la interfaz cuándo un flujo o prueba de carga masiva se encuentra en plena ejecución.
- Sistema de diseño visual unificado: Implementación de una nueva arquitectura de componentes visuales homogéneos con soporte en paralelo para temas claro y oscuro. Se unificó la estética de paneles, botones de acción, campos de texto con iluminación focal activa, tipografías técnicas optimizadas y filas de progreso de tareas. Toda la estructura y reglas de diseño se consolidaron en un nuevo manual de lineamientos visuales del producto.
...
- Transición visual armónica y equilibrio de interfaz: Se optimizó la transición estética desde la pantalla de bienvenida hacia el entorno principal de trabajo, incorporando una entrada fluida y una línea estilizada de alta tecnología en la barra superior. La pantalla de carga ahora adapta sus colores de forma nativa al modo claro, y todos los módulos, consolas, botones principales e indicadores de validación adoptan un equilibrio visual estructurado que prioriza la estabilidad de la pantalla y destaca los elementos de respuesta activa.
- Modernización e integración global de componentes: Se realizó una migración masiva de la interfaz hacia los nuevos componentes estandarizados de la aplicación. Esto incluye la renovación de más de 25 controles interactivos de configuración, todos los cuadros de diálogo (ventanas de confirmación, ofertas de planes, asistentes de creación y manuales), formularios de automatización móvil, barras de herramientas avanzadas y accesos de asistencia técnica, garantizando una experiencia de navegación uniforme y veloz.

### Corregido

- Optimización del flujo de configuración: La pantalla de bienvenida se reservó exclusivamente para el arranque inicial del software, eliminando su activación al abrir las preferencias. Ahora, al modificar el tema visual o activar el modo tecnológico, el sistema muestra un breve mensaje de transición («Aplicando preferencias...»). Adicionalmente, el panel de configuración se fijó en la parte superior con desplazamiento automatizado al ingresar, recuperando un diseño ampliado que optimiza drásticamente la lectura de opciones.
- Fluidez total en manuales de inicio rápido: Se reajustó la anchura del panel interactivo de guías para maximizar el área de lectura y comodidad visual. El contenido de asistencia técnica ahora se almacena previamente en la memoria caché local al ingresar a la sección principal, eliminando cualquier tiempo de espera o indicador de carga al alternar entre las guías de automatización móvil, escritorio o servicios.
...
- Ajuste estético en controles críticos (Tema Oscuro): Se optimizó la paleta visual para las acciones delicadas en el modo oscuro. El botón para Eliminar proyecto ahora utiliza una tonalidad roja mate y sutil que se integra de manera homogénea con el diseño sin generar competencias visuales con el resto de la interfaz, manteniendo intacto el contraste habitual en los paneles de alerta de errores para garantizar su perfecta lectura.
